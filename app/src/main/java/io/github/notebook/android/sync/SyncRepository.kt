...
class SyncRepository(context:Context, private val dao:NotebookDao) {
    ...
    private suspend fun persistDraft(draft:NoteEntity):NoteEntity=saveMutex.withLock{
        ...
        dao.put(saved);dao.deleteDraft(draft.id);if(saved.deletedAt!=null||saved.completedAt!=null||saved.reminderAt==null)Reminders.cancel(appContext,saved.id)else Reminders.schedule(appContext,saved.id,saved.title,saved.reminderAt,saved.recurrence);saved
    }
    ...
    suspend fun saveStep(step:TodoStepEntity){dao.putStep(step);dao.get(step.noteId)?.let{save(it)}}
    suspend fun deleteStep(id:String){val noteId=dao.getStep(id)?.noteId;dao.deleteStep(id);noteId?.let{dao.get(it)?.let{n->save(n)}}}
    suspend fun attach(noteId:String,uri:android.net.Uri):AssetEntity=withContext(Dispatchers.IO){
        ...
        val kind=when{mime.startsWith("image/")->"image";mime.startsWith("audio/")->"audio";else->"file"};AssetEntity(id,noteId,kind,name,mime,relative,target.absolutePath,sha(target.readBytes()),target.length(),true).also{dao.putAssets(listOf(it));dao.get(noteId)?.let{n->save(n)}}
    }
    ...
    private suspend fun pullNotes(c:ChannelSftp,s:SshSettings){
        ...
        idx["entries"]?.asJsonArray?.forEach { e ->
            val id=e.asJsonObject.str("noteID");val remoteVersion=e.asJsonObject["version"].asLong;val local=dao.get(id)
    ...
    private suspend fun pullAssets(c:ChannelSftp,s:SshSettings){
        ...
        val known=dao.allNotes().flatMap{dao.assets(it.id)}.associateBy{it.relativePath}
    ...
    private suspend fun pushNotesUnlocked(c:ChannelSftp,s:SshSettings){
        ...
        val confirmed=mutableListOf<Pair<String,Long>>();dao.dirtyNotes().filter{!it.conflict}.forEach{n->
    ...
    private suspend fun toEnvelope(n:NoteEntity):JsonObject { val noteAssets=dao.assets(n.id);val noteSteps=dao.steps(n.id);val textBlocks=BlockDocumentCodec.encodeMarkdown(n.body);val meta=JsonObject().apply{addProperty("id",n.id);addProperty("title",n.title);addProperty("createdAt",swiftDate(n.createdAt));addProperty("updatedAt",swiftDate(n.updatedAt));n.folderId?.let{addProperty("folderID",it)};addProperty("folderName",n.folderName);n.reminderAt?.let{addProperty("reminderAt",swiftDate(it))};addProperty("recurrenceRule",n.recurrence);addProperty("version",n.version);add("tagIDs",JsonArray().apply{n.tagIds.split(',').filter{it.isNotBlank()}.forEach{add(it)}});addProperty("viewMode","text");addProperty("blockCount",textBlocks.size()+noteAssets.size);addProperty("assetCount",noteAssets.size);addProperty("documentPath","Notes/${n.id}/document.json");addProperty("itemType",n.itemType);n.dueAt?.let{addProperty("todoDueAt",swiftDate(it))};n.completedAt?.let{addProperty("todoCompletedAt",swiftDate(it))};addProperty("todoIsImportant",n.important);addProperty("todoOrder",0);add("linkedNoteIDs",JsonArray())}
    ...
    private fun atomicWrite(c:ChannelSftp,p:String,text:String){val tmp="$p.tmp.android";c.put(ByteArrayInputStream(text.toByteArray()),tmp);c.rename(tmp,p)}
