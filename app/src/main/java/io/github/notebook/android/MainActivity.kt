...
@Composable private fun UpdatePrompt(){val context=LocalContext.current;val scope=rememberCoroutineScope();var release by remember{mutableStateOf<GithubRelease?>(null)};var message by remember{mutableStateOf<String?>(null)};var downloaded by remember{mutableStateOf<java.io.File?>(null)};var dismissed by remember{mutableStateOf(false)};LaunchedEffect(Unit){runCatching{AppUpdater.latestRelease()}.onSuccess{release=it}}
    if(release!=null&&!dismissed)AlertDialog(onDismissRequest={dismissed=true},icon={Icon(Icons.Default.SystemUpdate,null)},title={Text("发现新版本 ${release!!.version}")},text={Column(verticalArrangement=Arrangement.spacedBy(8.dp)){Text("当前版本：${BuildConfig.VERSION_NAME}");release!!.notes.take(600).takeIf{it.isNotBlank()}?.let{Text(it,style=MaterialTheme.typography.bodySmall)};message?.let{Text(it,color=if(it.startsWith("失败"))MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.primary)}}},confirmButton={Button(onClick={scope.launch{if(downloaded==null){message="正在下载并校验安装包…";runCatching{AppUpdater.download(context,release!!)}.onSuccess{downloaded=it;message="下载完成，正在打开系统安装程序";if(!AppUpdater.install(context,it))message="请允许安装未知应用，返回后再次点击安装"}.onFailure{message="失败：${it.localizedMessage}"}}else if(!AppUpdater.install(context,downloaded!!))message="请允许安装未知应用，返回后再次点击安装"}},enabled=message!="正在下载并校验安装包…"){Text(if(downloaded==null)"下载并升级" else "继续安装")}},dismissButton={TextButton({dismissed=true}){Text("稍后")}})}
