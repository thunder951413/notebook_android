...
WebDAV 目录使用 Android 原生 XML 解析器读取；解析失败会明确报错，避免将异常响应误判为空仓库并反复全量上传。同步会显示当前阶段和上传进度。兼容桌面旧数据迁移产生的 `legacy-asset:页面:附件` 与 `:sidecar:序号` 附属文件 ID，并保留路径校验。待上传附件的本地绝对路径失效时，WebDAV 会先检查应用附件目录，再按 SHA-256 从云端恢复已有对象，校验成功才继续提交。若本机和云端都无法恢复附件，界面保留完整错误与重试入口，待同步内容保持不变；恢复原附件后可重试。此类缺失文件错误不会触发后台连续重试。
